...
plt.errorbar(x, data.iloc[0, 1:].values.astype(np.float), data.iloc[1, 1:].values.astype(np.float), label="Shure SM57",fmt='rD', ecolor='r', elinewidth=1.5, capsize=15)
plt.errorbar(x, data.iloc[2, 1:].values.astype(np.float), data.iloc[3, 1:].values.astype(np.float), label="Rode NT2", fmt='gD', ecolor='g', elinewidth=1.5, capsize=15)
plt.errorbar(x, data.iloc[4, 1:].values.astype(np.float), data.iloc[5, 1:].values.astype(np.float), label="Piezo", fmt='bD', ecolor='b', elinewidth=1.5, capsize=15)
